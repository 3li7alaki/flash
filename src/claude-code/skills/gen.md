...
User says `/flash gen`, "generate cards about...", "make flashcards for...", "create cards from...", or similar.

## Instructions

1. **Determine the source.** The user may provide:
   - A topic as an argument: `/flash gen "Rust lifetimes"`
   - A file path: `/flash gen --from ./notes.md`
   - Conversation context (e.g., they were just reading code or discussing a concept)

2. **Run the generator.** Use the CLI:

   ```bash
   flash gen "topic" --count 10
   ```

   Or with a file:

   ```bash
   flash gen --from <file-path> --count 10
   ```

   If the source is conversation context, extract the key content into a temporary topic description and use:

   ```bash
   flash gen "detailed topic description based on conversation"
   ```
...
6. **Confirm save.** Let the user know where the deck was saved and how many cards it contains. Mention they can review it with `/flash review <deck-name>`.
